...
static void add_button(const char* button, unsigned int pos, bool fn_pressed = false) 
{
	sscanf(button, "%x:%s", &xhc.buttons[pos].code, xhc.buttons[pos].pin_name);
	xhc.buttons[pos].fn_pressed = fn_pressed;
}

static void button_setup() 
{
	unsigned int i = 0x01;
	add_button("0x01:reset", i++);
	add_button("0x02:stop", i++);
	add_button("0x03:start_pause", i++);
	add_button("0x04:feed+", i++);
	add_button("0x05:feed-", i++);
	add_button("0x06:spindle+", i++);
	add_button("0x07:spindle-", i++);
	add_button("0x08:machine_home", i++);
	add_button("0x09:safe-z", i++);
	add_button("0x0A:workpiece_home", i++);
	add_button("0x0B:toggle_spindle", i++);
	add_button("0x0D:probe_z", i++);
	add_button("0x0E:continuous", i++);
	add_button("0x10:macro10", i++);
	add_button("0x11:step", i++);
	add_button("0x04:macro1", i++, true);
	add_button("0x05:macro2", i++, true);
	add_button("0x06:macro3", i++, true);
	add_button("0x07:macro4", i++, true);
	add_button("0x08:macro5", i++, true);
	add_button("0x09:macro6", i++, true);
	add_button("0x0A:macro7", i++, true);
	add_button("0x0B:macro8", i++, true);
	add_button("0x0D:macro9", i++, true);
}
...
{
	fprintf(stderr, "%s by Frederic RIBLE (frible@teaser.fr)\n", name);
    fprintf(stderr, "\n");
...
void cb_response_in(struct libusb_transfer *transfer)
{
	...
	if (transfer->actual_length > 0) {
		...
		xhc.fn_button_code = in_buf[3];
		xhc.axis = (xhc_axis_t)in_buf[5];
		xhc.feed = (xhc_feed_t)in_buf[4];
		...
		*(xhc.hal->jog_enable_a) = (xhc.axis == axis_a);
		...
		for (i=0; i<NB_MAX_BUTTONS; i++) {
			...
			*(xhc.hal->button_pin[i]) = (xhc.button_code == xhc.buttons[i].code && !xhc.buttons[i].fn_pressed) || (xhc.fn_button_code == xhc.buttons[i].code && xhc.buttons[i].fn_pressed);
            if (strcmp("button-zero", xhc.buttons[i].pin_name) == 0) {
